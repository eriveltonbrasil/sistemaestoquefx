...
public class FormularioClientesController {
    ...
    // Campo de pesquisa da aba 2
    @FXML private TextField txtPesquisa;

    @FXML private TableView<Clientes> tabelaClientes;
    @FXML private TableColumn<Clientes, Integer> colCodigo;
    @FXML private TableColumn<Clientes, String> colNome;
    @FXML private TableColumn<Clientes, String> colRg;
    @FXML private TableColumn<Clientes, String> colCpf;
    @FXML private TableColumn<Clientes, String> colEmail;
    @FXML private TableColumn<Clientes, String> colTelefone;
    @FXML private TableColumn<Clientes, String> colCelular;
    @FXML private TableColumn<Clientes, String> colCep;
    @FXML private TableColumn<Clientes, String> colEndereco;
    @FXML private TableColumn<Clientes, Integer> colNumero;
    @FXML private TableColumn<Clientes, String> colComplemento;
    @FXML private TableColumn<Clientes, String> colBairro;
    @FXML private TableColumn<Clientes, String> colCidade;
    @FXML private TableColumn<Clientes, String> colUf;
    ...
    public void listarTabela() {
        ...
        ObservableList<Clientes> dadosDaTabela = FXCollections.observableArrayList(lista);
        tabelaClientes.setItems(dadosDaTabela);
    }

    private void preencherDadosFormulario() {
        Clientes obj = tabelaClientes.getSelectionModel().getSelectedItem();
    ...
    void novoAction(ActionEvent event) {
        limparCampos();
        tabPane.getSelectionModel().select(0); // Volta pra primeira aba
    }
    ...
    void salvarAction(ActionEvent event) {
        try {
            ...
            if (cbxEstado.getValue() != null) {
                obj.setEstado(cbxEstado.getValue());
            } else {
                obj.setEstado("");
            }
            ...
            alertaErro.setHeaderText("Erro ao salvar o cliente");
    ...
    void editarAction(ActionEvent event) {
        if (txtCodigo.getText().isEmpty()) {
            ...
            alerta.setHeaderText(null);
            alerta.setContentText("Por favor, selecione um cliente na aba de Consulta antes de editar!");
            alerta.showAndWait();
            return;
        }
        ...
        try {
            ...
            alerta.setTitle("Sucesso!");
            alerta.setHeaderText(null);
            alerta.setContentText("Cliente editado com sucesso!");
            alerta.showAndWait();
            ...
            Alert alertaErro = new Alert(Alert.AlertType.ERROR);
            alertaErro.setTitle("Atenção");
            alertaErro.setHeaderText("Erro ao editar o cliente");
            alertaErro.setContentText(e.getMessage());
            alertaErro.showAndWait();
        }
    }
    ...
    void excluirAction(ActionEvent event) {
        if (txtCodigo.getText().isEmpty()) {
            ...
            alerta.setTitle("Atenção");
            alerta.setHeaderText(null);
            alerta.setContentText("Por favor, selecione um cliente na aba de Consulta antes de excluir!");
            alerta.showAndWait();
            return;
        }
        ...
        confirmacao.setContentText("Tem certeza que deseja excluir o cliente " + txtNome.getText() + "?");
        ...
        if (resultado.isPresent() && resultado.get() == ButtonType.OK) {
            try {
                ...
                alerta.setTitle("Sucesso!");
                alerta.setHeaderText(null);
                alerta.setContentText("Cliente excluído com sucesso!");
                alerta.showAndWait();
    ...
    // --- BOTÃO PESQUISAR (ABA CONSULTA) ---
    @FXML
    void pesquisarAction(ActionEvent event) {
        // Pega o nome e coloca a % para o banco achar partes da palavra
        String nomePesquisa = "%" + txtPesquisa.getText() + "%";
        
        ClientesDAO dao = new ClientesDAO();
        List<Clientes> lista = dao.Filtar(nomePesquisa); // Puxa do banco
        
        // Joga a lista filtrada na tabela
        ObservableList<Clientes> dadosDaTabela = FXCollections.observableArrayList(lista);
        tabelaClientes.setItems(dadosDaTabela);
    }
